...
function toggleMenu(event){
  ...
  if (active) {
    ...
    // Remova a classe 'container-header' quando 'active' for verdadeiro
    document.querySelector('nav').classList.remove('container-header');


  } else {
    event.currentTarget.setAttribute('aria-label', 'Abrir Menu');
     
    // Adicione a classe 'container-header' quando 'active' for falso (se necessário)
     document.querySelector('nav').classList.add('container-header');
  } 
}
...
const logoTop = document.querySelector(".logo-mergu-link-scroll");

function downLogo() {
  
  logoTop.classList.add('ativarScroll');
 
}

window.addEventListener('scroll', downLogo);
